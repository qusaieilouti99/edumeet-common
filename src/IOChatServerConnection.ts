...
interface ClientServerEvents {
	...
	request: (request: ChatSocketMessage, result: (
...
export class IOChatServerConnection extends BaseConnection<ChatSocketMessage> {
	public closed = false;
	...
	@skipIfClosed
	public notify(notification: ChatSocketMessage): void {
	...
	@skipIfClosed
	private sendRequestOnWire(socketMessage: ChatSocketMessage): Promise<unknown> {
	...
	@skipIfClosed
	public async request(request: ChatSocketMessage): Promise<unknown> {
